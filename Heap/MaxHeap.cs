...
namespace Heap
{
    class MaxHeap <T> where T : IComparable
    {
        ...
        private bool hasParent(int child) => (child > 0) ? (getParentIndexOf(child) >= 0) : false;
        ...
        private void heapifyDown()
        {
            ...
            {
                int greaterIndex = getLeftChildIndexOf(idx);
                if (hasRightChild(idx) && (RightChildOf(idx).CompareTo(LeftChildOf(idx)) > 0))
                {
                    greaterIndex = getRightChildIndexOf(idx);
                }
                if (arr[idx].CompareTo(arr[greaterIndex]) < 0)
                {
                    swap(idx, greaterIndex);
                }
                else
                {
                    break;
